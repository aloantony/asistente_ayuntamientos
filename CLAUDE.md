...
- Docs: `docs/` is written in Spanish; the living docs (`arquitectura.md`, `requisitos.md`) carry `Actualizado: <fecha>` headers. Decisions are recorded as numbered ADRs in `docs/decisiones.md`. `README.md` is the source of operational detail. Every non-documentation commit is followed immediately by a dedicated docs sync commit that updates the relevant living doc, ADR or README section; if no product, architecture, operations or requirements text changes, document that explicitly in the docs sync commit instead of leaving the code commit unpaired.
