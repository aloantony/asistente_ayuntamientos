...
def require_administration_permission(
    db: Session,
    current_user: User,
    organization_id: int,
    permission_code: str,
...
def get_administration_organization_for_read(
    db: Session,
    organization_id: int,
) -> Organization:
    ...
    if organization.status not in {"active", "paused"}:
        raise HTTPException(
            ...
            detail="Organization is not available for administration reads",
        )
    return organization


def get_administration_organization_for_write(
    db: Session,
    organization_id: int,
) -> Organization:
