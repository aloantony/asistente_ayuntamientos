...
def get_task_organization_for_read(
    db: Session,
    organization_id: int,
) -> Organization:
    ...
    if organization.status not in {"active", "paused"}:
        raise HTTPException(
            ...
            detail="Organization is not available for roadmap reads",
        )
    return organization


def get_task_organization_for_write(
    db: Session,
    organization_id: int,
) -> Organization:
    organization = get_existing_organization(db, organization_id)
    if organization.status != "active":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Organization must be active to modify the roadmap",
        )
    return organization
