...
def web_search_unavailable_agent_key(allowed_agents: list[AgentSpec]) -> str:
...
def update_pending_work_from_assistant_reply(
    ...
    if isinstance(state.get("pending_requirement_proposal"), dict):
        return
...
def semantic_plan_ordinance_input(
    ...
    if plan.intent != "read_ordinances":
        return None
    if plan.action != "semantic_search_ordinances":
        return None
...
def semantic_plan_requirements_organization(
    ...
    if plan.intent != "read_requirements":
        return None
    if plan.action != "list_requirements":
        return None
    if plan.confidence < 0.5:
        return None
...
def semantic_plan_create_requirement_target(
    ...
    if plan.intent != "create_requirement":
        return None
    if plan.action != "create_requirement":
        return None
    if plan.confidence < 0.5:
        return None
...
def remember_model_tool_result(
    state: dict,
    *,
    tool_name: str,
    tool_input: dict,
    ...
    organization = None
    raw_organization_id = tool_input.get("organization_id")
    if raw_organization_id is not None:
        try:
            organization = organization_by_id(organizations, int(raw_organization_id))
        except (TypeError, ValueError):
            organization = None
    organization = organization or selected_or_single_organization(organizations, state)

    result = decode_tool_json(result_content) if ok else None
    ...
    if organization is not None:
        action_state["organization_id"] = organization.id
        record_selected_organization(state, organization)
    state["last_direct_action"] = action_state
...
def classify_turn_intent(text: str) -> TurnIntent:
    if is_ordinance_request(text):
        return TurnIntent("unknown", "ordinance_model_decision")
    return TurnIntent("unknown", "model_decision")
...
def try_handle_direct_turn(
    ...
    if not isinstance(pending_work, dict):
        pending_work = None
    pending_requirement_proposal = state.get("pending_requirement_proposal")
    if not isinstance(pending_requirement_proposal, dict):
        pending_requirement_proposal = None
    ...
    semantic_plan = plan_turn(
        ...
        context={
            ...
            "pending_requirement_proposal": pending_requirement_proposal,
        },
    )
    turn_intent = (
    ...
    if is_web_search_request(user_text) and not web_search_is_available(
        ...
        return persist_direct_prompt(
            ...
            allowed_agents,
            state,
            agent_key=web_search_unavailable_agent_key(allowed_agents),
    ...
    if (
        (
            ...
            or pending_work is not None
            or pending_requirement_proposal is not None
        )
    ...
    if pending_requirement_proposal is not None and is_negative(user_text):
        state.pop("pending_requirement_proposal", None)
        return persist_direct_prompt(
            db,
            conversation,
            ...
            state,
            agent_key="requirements_intake",
            content="De acuerdo, no guardo esa propuesta de necesidad.",
            reason="pending_requirement_proposal_cancelled",
        )
    ...
    if pending_work is None and accepts_proposed_requirement_draft(user_text):
        ...
        if (
            pending_requirement_proposal is None
            and requirement_draft_is_complete(legacy_draft)
