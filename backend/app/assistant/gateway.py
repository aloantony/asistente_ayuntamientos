...
def _from_openai_response(response_data: dict) -> AICompletion:
    ...
    cleaned_content, inline_tool_calls = _extract_inline_hermes_tool_calls(raw_content)
    ...
    return AICompletion(
        model=response_data.get("model") or settings.assistant_model,
...
def _extract_inline_hermes_tool_calls(
    content: str,
    ...
    cleaned_content = _HERMES_TOOL_CALL_RE.sub("", content).strip()
...
def _parse_tool_call_payload(payload: str) -> AIToolUseBlock | None:
    try:
        ...
        logger.warning("Hermes emitted a malformed inline tool call")
        return None

    function = parsed.get("function") or {}
    name = parsed.get("name") or parsed.get("tool_name") or function.get("name")
    if not name:
        logger.warning("Hermes emitted an inline tool call without a name")
        return None
...
def _decode_tool_arguments(arguments) -> dict:
    ...
    if isinstance(arguments, str):
        try:
            ...
            logger.warning("AI provider emitted malformed tool arguments")
            return {}
