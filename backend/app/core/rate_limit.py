...
class InMemorySlidingWindowRateLimiter:
    """Thread-safe per-process window.

    Two callers: the RATE_LIMIT_BACKEND=memory fallback for development and
    tests, and the reference-map throttles in `app/reference_layers/
    wms_routes.py`, which are per-user comfort limits on a cached upstream
    rather than a credential budget and so do not need to cross workers.
