...
def get_plenos_organization_for_read(
    db: Session,
    organization_id: int,
) -> Organization:
    ...
    if organization.status not in {"active", "paused"}:
        raise HTTPException(
            ...
            detail="Organization is not available for plenos reads",
        )
    return organization


def get_plenos_organization_for_write(
    db: Session,
    organization_id: int,
) -> Organization:
