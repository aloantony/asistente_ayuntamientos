...
only one row per layer and snapshot.  That made a reviewed strategy impossible
to evolve (and made its synthetic generation-one blockers impossible to
supersede) without deleting evidence.

This successor keeps every historical row and changes the identity to
provider/snapshot/generation/layer.  Composition dependencies are accepted only
when their target strategy belongs to the same snapshot and generation.
"""
