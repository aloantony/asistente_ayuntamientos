...
def test_prompt_requires_page_reading_and_exact_final_citations():
    assert "`read_web_page` aparece entre las herramientas" in ICONCEJO_SYSTEM_PROMPT
    assert "No afirmes haber leído una página" in ICONCEJO_SYSTEM_PROMPT
    assert "cita su `final_url`" in ICONCEJO_SYSTEM_PROMPT
    assert "muestra también su `source_url`" in ICONCEJO_SYSTEM_PROMPT
