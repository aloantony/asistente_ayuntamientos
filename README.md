...
Hermes Agent also runs the semantic planner/router with `ASSISTANT_PLANNER_RUNTIME=hermes_agent`. For `ASSISTANT_RUNTIME=hermes_agent`, the backend treats the planner as effectively enabled even if an older local `.env` still says `disabled`, because otherwise the assistant falls back to rigid shortcuts and feels scripted. Before operational routing, the backend still classifies safe direct intents with `TurnIntent` (`global_capabilities`, `read_requirements`, `create_requirement`, `create_test_requirement`, `unknown`). Global capability questions such as “¿qué puedes hacer?” are answered from a stable product-level contract, not from whichever internal agent would otherwise be selected. In planner mode Hermes only chooses/structures the backend path for turns that still need an operational agent (`requirements_intake` or `consultation`) or a safe action policy; the backend still filters tools, validates RBAC, executes actions and stores the audit trail. If the planner is unavailable, the assistant falls back safely, but mayor-facing deployments should treat that as a degraded state.
