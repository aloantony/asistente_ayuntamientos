...
Chat messages may explicitly reference up to `ASSISTANT_MAX_ATTACHMENTS_PER_MESSAGE` existing project documents, or upload a document through the same controlled `Document` storage first. Only bounded text extracted from TXT, PDF, DOCX and XLSX is placed in provider memory for that single turn; binaries are never sent, and images are shown with `vision_unavailable` until an approved vision provider exists. The durable message relation stores only document identity, order and extraction status/count—not extracted text. Attachment turns expose only read-only, non-web and non-memory tools. PDF, DOCX and XLSX parsing runs in an isolated `python -I` subprocess with timeout, Linux resource limits, signature validation and ZIP expansion controls; bounded plain-text decoding remains local. Extracted content is untrusted data, never instructions. Per-file, aggregate and extraction-byte limits are configured with `ASSISTANT_ATTACHMENT_MAX_CONTEXT_CHARS`, `ASSISTANT_ATTACHMENT_TOTAL_CONTEXT_CHARS` and `ASSISTANT_ATTACHMENT_MAX_EXTRACT_BYTES`.
