...
Decisiones de fondo en ADR-031 (topología y TLS), ADR-032 (endurecimiento) y
ADR-033 (copias). Este documento es la receta; los ADR explican el por qué.
...
| Backups o snapshots en la consola de Hetzner | **Obligatorio**: son la única copia fuera de la máquina (ADR-033) |
...
`ALLOWED_HOSTS=*` (ADR-032): si falla al levantar, lee el mensaje, es literal.
...
Los contenedores ya **no corren como root** (ADR-032). Si el volumen
...
y retención. Sin ellos no hay ninguna copia fuera de la máquina (ADR-033).
...
en el bundle del navegador durante el build (ADR-031).
