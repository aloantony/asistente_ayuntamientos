...
El lector no es un navegador ni un proxy general. Ejecuta un GET anónimo sin cookies, autenticación, JavaScript ni compresión, limita tipos a HTML/XHTML o texto y deshabilita PDF remoto hasta disponer de extracción aislada. Solo permite egreso por los puertos 80/443 y acota bytes, texto, redirecciones y tiempo total. Rechaza credenciales en URL, nombres internos, endpoints de metadatos y cualquier IP que no sea pública. Resuelve DNS en un subproceso acotado y cancelable, valida todas las respuestas antes de conectar y fija el socket a una IP validada manteniendo Host, SNI y verificación de certificado sobre el hostname. Cada redirección debe conservar exactamente esquema, hostname y puerto efectivo; si cambia el origen, el modelo debe buscar y autorizar la nueva URL. Títulos, snippets y texto extraído se consideran entrada externa no confiable: además del prompt, `ToolContext` marca cualquier búsqueda/lectura correcta y el servidor deniega toda herramienta mutante durante el resto del turno. Realtime persiste únicamente ese booleano de seguridad entre sus llamadas, nunca el cuerpo.
