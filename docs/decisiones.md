...
## ADR-029: Adjuntos del chat autorizados para un único turno (2026-07-16)
...
## ADR-030: Lectura web con procedencia efímera y defensa SSRF (2026-07-16)

La búsqueda y la lectura se mantienen como herramientas separadas. `web_search` continúa usando un único proveedor seleccionado explícitamente conforme a ADR-022; este cambio no añade fallback automático. Solo las URLs incluidas en el resultado compacto que el modelo recibió se incorporan a una allowlist en memoria del turno. `read_web_page` exige coincidencia exacta después de normalizar y no puede reutilizar esa procedencia en otro mensaje. En Realtime la allowlist se reconstruye únicamente desde búsquedas correctas del turno de voz activo.

El lector no es un navegador ni un proxy general. Ejecuta un GET anónimo sin cookies, autenticación, JavaScript ni compresión, limita tipos a HTML/XHTML, texto y PDF, y acota bytes, páginas, texto, redirecciones y tiempo total. Rechaza credenciales en URL, nombres internos, endpoints de metadatos y cualquier IP que no sea pública. Valida todas las respuestas DNS antes de conectar, fija el socket a una IP validada manteniendo Host, SNI y verificación de certificado sobre el hostname, repite la validación en cada redirección y prohíbe degradar HTTPS a HTTP. El texto se considera entrada externa no confiable y el prompt impide obedecer sus instrucciones.

El resultado completo se entrega al modelo solo dentro del bucle del turno y hasta el límite configurado. La actividad persistida conserva URL de origen/final, título, tipo, redirecciones, número de caracteres y un preview corto, no la página completa. Así sigue siendo posible auditar qué fuente se leyó y mostrarla en la interfaz sin convertir el historial en una copia durable de contenidos externos.
