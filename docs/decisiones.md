...
El usuario puede seleccionar documentos ya accesibles de un proyecto o subirlos mediante el almacenamiento `Document` existente. La autorización se valida por documento y tenant antes de persistir el mensaje; un identificador ausente o ajeno devuelve el mismo 404 genérico. La relación durable conserva únicamente el documento, posición y estado/conteo de extracción para que la interfaz pueda representar el adjunto. El texto extraído nunca se guarda en mensajes ni en la tabla de relación: existe solo en memoria durante la petición que lo autorizó y no reaparece en el historial de turnos posteriores.

TXT, PDF, DOCX y XLSX se leen localmente con límites de bytes, caracteres y estructura; se normalizan caracteres Unicode de control. Los binarios nunca se envían al modelo. Las imágenes se conservan y previsualizan desde la descarga autenticada, pero se marcan `vision_unavailable` y no aportan contexto mientras no exista un proveedor de visión aprobado. Todo texto extraído se considera contenido no fiable, no instrucciones. En cualquier turno con adjuntos se eliminan por código `web_search`, `read_web_page` y `propose_memory_entry` de las herramientas disponibles para impedir egreso o memoria automática; las herramientas municipales siguen sujetas a RBAC, tenancy y confirmación normal.
