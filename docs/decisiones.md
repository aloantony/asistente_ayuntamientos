...
La supervisión humana no depende del prompt: `create_requirement` queda bloqueada si el modelo intenta crear un borrador en el mismo turno en que lo propone. El backend guarda `pending_confirmation` en `assistant_conversations.state`, exige una respuesta natural explícita en un turno posterior y compara un digest SHA-256 del nombre de herramienta y de la carga completa. El consumo se serializa con bloqueo de fila y se confirma antes de ejecutar, de modo que es de un solo uso incluso si la herramienta hace rollback o llegan dos consumidores concurrentes. Los ejecutores siguen aplicando RBAC/tenancy y dejan rastro en `assistant_messages.actions`.
...
Cada invitación usa 32 bytes aleatorios, caduca por configuración y guarda únicamente el digest SHA-256. El enlace entrega el secreto en el fragmento de URL (`#token`), que el navegador no envía al servidor frontend ni como referencia; la pantalla lo retira del historial y lo conserva temporalmente en `sessionStorage` si debe pasar por el login. Hasta incorporar un proveedor de correo transaccional, el administrador recibe el enlace una sola vez para remitirlo por un canal acordado.

El token manual no demuestra por sí solo posesión del email porque el administrador que invita también lo ve. Por ello la aceptación exige una sesión activa cuyo email coincida, inserta exclusivamente la membresía de esa identidad y marca el token como consumido bajo bloqueo PostgreSQL. No consulta públicamente si la identidad existe ni permite crear usuarios, elegir contraseñas o modificar campos globales. El alta de una identidad nueva queda reservada al superusuario hasta disponer de entrega server-side por email o IdP; así se evita pre-hijacking entre tenants.

Las invitaciones terminales se conservan por ahora como historial administrativo y contienen el email invitado. Antes de producción debe acordarse con el responsable de tratamiento un plazo de conservación y un proceso de purga; no se fija automáticamente desde código porque depende de la política municipal y de sus obligaciones de auditoría.
...
## ADR-025: Rate limiting distribuido con Redis (2026-07-10)

Los presupuestos de intentos de `POST /auth/login` y `POST /auth/change-password` pasan del proceso FastAPI a Redis para mantener una única decisión al desplegar varios workers o réplicas. Se conserva la semántica de ADR-015: ventana deslizante, reserva antes de Argon2 y reembolso tras una credencial válida, de modo que solo el fallo consume cupo. La adquisición, poda, comprobación y alta se ejecutan en un script Lua atómico; el reembolso y reajuste de TTL se ejecutan en otro. El reloj usado es el de Redis para evitar divergencias entre hosts.

La clave lógica sigue siendo cliente+cuenta para login y usuario para cambio de contraseña, pero Redis nunca recibe esos valores en claro. El backend deriva una clave HMAC-SHA256 con `SECRET_KEY`, añade un namespace sin datos personales y usa UUID aleatorios como miembros del conjunto ordenado. No se registran claves lógicas, contraseñas ni URLs Redis en errores.

Una caída de Redis bloquea la operación de autenticación con `503`: continuar sin presupuesto compartido abriría un bypass precisamente durante una incidencia. `RATE_LIMIT_BACKEND=memory` existe solo como fallback explícito de `development` y `test`; la validación de configuración rechaza ese backend en `production`. Redis tiene healthcheck en Compose y timeouts breves configurables. Fuera de loopback, producción exige TLS con certificado y hostname verificados, autenticación fuerte y una URL sin parámetros capaces de sobreescribir esos timeouts. Sigue pendiente evaluar un presupuesto secundario anti-spraying con métricas reales para no penalizar a ayuntamientos que comparten proxy.
