...
El bloque de temperatura queda **pendiente de fuente de datos**: el interruptor se persiste y el bloque muestra un guion en vez de inventar una cifra. Cualquier implementación futura supondría un punto de egreso externo nuevo (no de IA, así que fuera del gateway de ADR-013/ADR-021) y debe decidirse explícitamente antes de añadirlo, junto con las coordenadas del municipio, que ya existen en `municipal_profiles` para no necesitar otra migración.
