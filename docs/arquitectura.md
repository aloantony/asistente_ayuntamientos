...
- `AssistantCanvasDocument` y sus revisiones inmutables representan borradores privados de una conversación. Pueden clasificarse con una organización accesible, pero no son `Document`, `Ordinance` ni contenido oficial compartido por el tenant.
...
- El Markdown del lienzo es otro dominio: se guarda en PostgreSQL, versionado y
  ligado al propietario de la conversación. Su contrato está en
  `docs/lienzo-documentos.md`.

## IA (dirección)

- La política declarativa separa `database_write/explicit`, con autorización de un solo uso, de `draft_write/direct`, limitado a borradores privados, no oficiales, versionados y reversibles del lienzo. La dirección de producto seguirá graduando autonomía por riesgo, competencia y delegación como define `docs/vision-producto.md`.
- Toda llamada a APIs externas de IA o a un runtime privado de agentes pasa por el gateway interno (`app/assistant/gateway.py`); el pipeline de voz STT/TTS egresa solo por `app/assistant/speech.py` (ADR-021) y las consultas web públicas solo por `app/assistant/web_search.py` (ADR-022). Pueden viajar el texto de conversación, memoria institucional aprobada, campos dictados por el usuario y el cuerpo de un borrador cuando Anacleto lo lee expresamente; también audio del turno, texto sintetizable y, para búsqueda, la consulta pública normalizada. Una edición manual del lienzo no provoca egreso. Los archivos originales de `Document` no salen del servidor. Los logs registran metadatos, nunca cuerpos del lienzo, conversación, consultas o resultados.
- Primera pieza implementada: Anacleto v2 (`app/assistant/`) es un único asistente model-first. No hay planner/router ni handlers de plantillas: el modelo redacta desde un system prompt con contrato de producto, organizaciones visibles, memoria aprobada, cobertura de ordenanzas y herramientas filtradas por permisos. El backend ejecuta las herramientas con los mismos chequeos RBAC que las rutas REST, persiste `agent_key="anacleto"`, `routing=null` y un rastro JSON redactado. `create_requirement` y otras escrituras oficiales conservan propuesta y autorización explícita one-shot; las herramientas del lienzo crean y revisan borradores directamente bajo bloqueo de turno y concurrencia optimista. El bucle de tool-use usa el runtime configurado (`ASSISTANT_RUNTIME=anthropic`, `ASSISTANT_RUNTIME=hermes_agent`, `ASSISTANT_RUNTIME=openai_responses` o, exclusivamente en desarrollo y con dos opt-in explícitos, `ASSISTANT_RUNTIME=codex_subscription`) y el egreso LLM pasa por `gateway.py`. Responses opera con `store=false`; PostgreSQL sigue siendo la fuente de verdad y el razonamiento cifrado necesario para encadenar herramientas solo vive durante el turno. El puente de suscripción mantiene una conexión efímera a `codex app-server` entre rondas y solo guarda en memoria un handle opaco; las herramientas continúan ejecutándose en el bucle propio. La evaluación Codex actual ejecuta el backend en el host aislado; el sidecar/socket no está implementado. Sin configuración completa del runtime seleccionado, el módulo queda deshabilitado (503).
- Streaming web: el endpoint SSE `/assistant/conversations/{id}/messages/stream` emite deltas de texto y actividad de herramientas; el POST clásico queda para compatibilidad y Telegram. Los cuerpos del lienzo se sustituyen por longitud y SHA-256 en eventos, acciones y estado Realtime. Tras leer un cuerpo o los extractos del historial se bloquean herramientas web durante el turno; el taint web bloquea a su vez las herramientas del lienzo.
...
- La biblioteca de usuario vive en `/ordenanzas` y usa `GET /ordinances/search` para paginar el corpus completo, con filtros territoriales, poblacionales, temáticos y ámbitos por fragmento, ordenanza o municipio. Solo recupera ordenanzas/chunks aprobados del modelo de embedding vigente; excluye por defecto derogadas, sustituidas y archivadas y comunica la cobertura incompleta de población. La ficha usa el detalle y los chunks aprobados; la comparación devuelve también los municipios solicitados sin filas para representar ausencias de cobertura. Las mutaciones validan el número total de chunks antes de sustituirlos; embeddings locales se completan tras el commit y cada embedding remoto pasa por un trabajo RQ idempotente, acotado y reintentable fuera de la petición HTTP. La creación importada de ordenanza, chunks e informe es atómica. Contrato completo: `docs/biblioteca-ordenanzas.md`.
- Las descargas normativas validan la definición de fuente, esquema/puerto/dominio, todas las IP resueltas y cada redirección; conectan al `sockaddr` ya validado, sin proxy ni redirecciones automáticas. Los documentos tenant-scoped no filtran id o metadatos desde el catálogo global.

## Frontend

- App Router multi-ruta con shell de navegación lateral: `/ayuntamiento`, `/ordenanzas`, `/asistente`, `/requisitos`, `/proyectos`, `/mapa`, `/cuenta` y `/admin/{producto,memoria,usuarios,grupos,organizaciones,roles,municipios,ordenanzas}`. Los ítems del menú usan los mismos predicados de permisos que las rutas; `/ayuntamiento` exige `municipalities.view|manage` y deriva su contexto de las organizaciones visibles de la sesión, `/ordenanzas` exige `ordinances.view|manage` y muestra la comparación solo con `ordinances.compare|manage`, producto exige superusuario, memoria exige `assistant.use` y `assistant.memory.review`, los permisos de solo lectura de ordenanzas no abren administración y `/proyectos` solo aparece con permisos de proyecto. El detalle de un activo en `/mapa` compone su panel de mantenimiento sin duplicar la ficha del inventario. Tras el login se aterriza en la primera sección visible.
- `/asistente` compone chat y lienzo documental contextual. El cliente valida acciones `ui.open_canvas_document`, ofrece editor/vista previa/historial, autoguarda con control de revisión y no renderiza imágenes Markdown remotas.
