...
- El catálogo de permisos se siembra automáticamente al arrancar el backend (idempotente); `POST /admin/permissions/bootstrap` sigue disponible como re-siembra manual. El arranque también siembra fuentes jurídicas oficiales mínimas para importación de ordenanzas, incluidos los BOP provinciales de Castilla y León.
...
## Ordenanzas Castilla y León

- El objetivo operativo es cubrir todas las ordenanzas municipales existentes de los pueblos de Castilla y León, empezando por cobertura medible por provincia. Burgos y Soria pueden coexistir como corpus ya cargados; el resto de provincias deben aparecer explícitamente como pendientes, no inferirse como cubiertas.
- Al arrancar, `backend/app/ordinances/seed.py` siembra como fuentes activas el BOE auxiliar y los BOP provinciales de Ávila, Burgos, León, Palencia, Salamanca, Segovia, Soria, Valladolid y Zamora. Para Burgos existe conector determinista; para el resto, mientras no haya conector específico provincial, los jobs usan URLs semilla oficiales o búsqueda Hermes Web restringida al dominio configurado.
...
- Go/no-go antes de una demo: verificar fuentes BOP activas en `official_legal_sources`, ordenanzas aprobadas, chunks `review_status='approved'`, embeddings `embedding_status='ready'` y búsquedas positivas con `semantic_search_ordinances`. Las búsquedas aceptan filtros estructurados por `municipality_id`/`municipality_name` y `topic`; además, el backend tiene una ruta determinista para preguntas de ordenanzas cargadas: detecta municipio/materia desde la DB y ejecuta `semantic_search_ordinances` sin depender de que el modelo emita tool-call nativo. Si no hay chunk aprobado para un municipio/materia, el asistente debe reconocer falta de cobertura sin pedir al usuario que reformule.
- La cobertura operativa se mide con `GET /ordinances/coverage/castilla-y-leon` y `GET /ordinances/coverage/provinces/{province}`. `GET /ordinances/coverage/burgos` sigue existiendo por compatibilidad y delega en el reporte genérico de provincia. Los embeddings fallidos se pueden reintentar por provincia con `POST /ordinances/coverage/provinces/{province}/retry-embeddings`; `POST /ordinances/coverage/burgos/retry-embeddings` se conserva como alias.
- Sprint 1 mantiene el conector determinista BOPBUR en `backend/app/ordinances/bop_burgos.py`: consulta el formulario oficial `/busqueda`, parsea anuncios/PDFs oficiales sin recurrir a búsqueda web general y alimenta `ordinance_import_jobs` cuando la fuente activa es `bopbur.diputaciondeburgos.es`. El reporte agrupa por nombre de municipio para evitar que altas duplicadas locales distorsionen la cobertura lógica.
