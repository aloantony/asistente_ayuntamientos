...
- Asistente conversacional (`app/assistant/`) con registro declarativo de agentes (`requirements_intake` y `consultation`) y catálogo backend de herramientas. Cada turno que necesita modelo selecciona un agente; el backend filtra las herramientas permitidas, aplica los mismos chequeos RBAC que las rutas REST y persiste `agent_key`, `routing` y el rastro JSON de herramientas.
- Antes del routing operativo, el backend clasifica intenciones conversacionales directas con `TurnIntent` (`global_capabilities`, `read_requirements`, `read_ordinances`, `read_map_items`, `capture_requirement`, `create_requirement`, `create_test_requirement`, `unknown`). Las intenciones globales, como “¿qué puedes hacer?” o “¿puedes crear requisitos?”, se responden con una visión de producto estable y no arrancan por sí solas un flujo de creación.
- Cuando `ASSISTANT_PLANNER_RUNTIME=hermes_agent`, Hermes Agent puede producir un `SemanticTurnPlan` estructurado (`intent`, `action`, `query`, `target`, `draft`, `reference`, `confidence`) para entender referencias naturales. El backend lo trata como propuesta interna: valida permisos, visibilidad y techo de herramientas antes de ejecutar nada. Las acciones de lectura de requisitos, ordenanzas y mapa, y la delegación a la oficina de agentes, pueden resolverse por políticas backend auditadas.
- La captura de necesidades sigue ADR-020. Si el usuario aporta una necesidad completa, Anacleto prepara una propuesta y el backend la guarda como `pending_work` en estado `awaiting_confirmation`; no se escribe todavía. Solo tras confirmación explícita, o una petición explícita de guardado con título y problema mínimos, el backend comprueba duplicados visibles, ejecuta `create_requirement` y crea el registro como borrador (`status=draft`, `source_type=conversation`). Si el modelo intenta llamar a `create_requirement` sin esa confirmación, la herramienta se rechaza y no se crea ningún registro.
- El agente de consulta puede buscar ordenanzas internas ya importadas/vectorizadas mediante una herramienta read-only (`semantic_search_ordinances`), protegida por `ordinances.compare`, para responder con fragmentos citables y fuente oficial sin buscar en internet ni inventar normativa.
- El bucle síncrono de tool-use usa el runtime configurado (`ASSISTANT_RUNTIME=anthropic` o `ASSISTANT_RUNTIME=hermes_agent`). En modo Hermes Agent, el backend llama al API Server privado compatible con OpenAI; Hermes Agent actúa como aplicación/runtime o planner, no como base de datos de memoria ni como autoridad de permisos. Conversaciones y mensajes persisten en PostgreSQL y son privados de su autor. Sin configuración completa del runtime seleccionado, el módulo queda deshabilitado (503).
- Feedback interno del asistente: Anacleto puede proponer `send_admin_feedback` cuando detecta una mejora o fricción, conserva el último feedback enviado como estado estructurado de conversación y permite explicar dónde lo revisan los superusuarios o convertirlo después en una necesidad municipal borrador si el usuario lo pide y tiene permisos.
...
- Go/no-go antes de una demo: verificar BOPBUR en `official_legal_sources`, ordenanzas BOPBUR aprobadas, chunks `review_status='approved'`, embeddings `embedding_status='ready'` y búsquedas positivas con `semantic_search_ordinances`. Las búsquedas aceptan filtros estructurados por `municipality_id`/`municipality_name` y `topic`; además, el backend tiene una ruta determinista para preguntas de ordenanzas cargadas: detecta municipio/materia desde la DB y ejecuta `semantic_search_ordinances` sin depender de que el modelo emita tool-call nativo. Si no hay chunk aprobado para un municipio/materia, el asistente debe reconocer falta de cobertura sin pedir al usuario que reformule.
