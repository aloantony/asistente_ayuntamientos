...
Aplicación multi-tenant con cinco servicios en Docker Compose:
...
- El límite de login por cliente+cuenta no agrega todavía un segundo presupuesto por cliente para detectar password spraying entre muchas cuentas; debe calibrarse con datos operativos para no convertir proxies municipales compartidos en un bloqueo global (ADR-023).
