...
- Toda llamada a APIs externas o inferencia remota de IA pasa por el gateway interno (`app/assistant/gateway.py`, punto único de salida): solo viaja el texto de la conversación, memoria institucional aprobada y los campos que el usuario dicta; los documentos originales no salen del servidor y los logs registran solo metadatos (proveedor, modelo, tokens), nunca contenido.
- Primera pieza implementada: el agente conversacional de intake de requisitos (`app/assistant/`). Bucle síncrono de tool-use contra el proveedor configurado (`ASSISTANT_PROVIDER=anthropic|hermes`; por defecto Claude con `claude-opus-4-8`; Hermes usa un endpoint remoto privado compatible con OpenAI). Las herramientas del agente ejecutan las mismas validaciones RBAC que las rutas REST, los requisitos se crean siempre como borrador con `source_type=conversation`, y cada mensaje del asistente guarda un rastro JSON de las herramientas ejecutadas. Conversaciones y mensajes persisten en PostgreSQL y son privados de su autor. Sin proveedor configurado el módulo queda deshabilitado (503); en producción Hermes exige `HERMES_REAL_DATA_ALLOWED=true` tras cerrar DPA/contrato y retención/transferencias.
