...
- Go/no-go antes de una demo: verificar BOPBUR en `official_legal_sources`, ordenanzas BOPBUR aprobadas, chunks `review_status='approved'`, embeddings `embedding_status='ready'` y búsquedas positivas con `semantic_search_ordinances`. Las búsquedas del asistente aceptan filtros estructurados por `municipality_id`/`municipality_name` y `topic`; cuando el usuario cite municipio o materia, el agente debe pasarlos para evitar que el ranking semántico local priorice otro municipio. Si no hay chunk aprobado para un municipio/materia, el asistente debe reconocer falta de cobertura.
